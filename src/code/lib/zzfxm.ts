    ...
    sequence.map((patternIndex, sequenceIndex) => {
      ...
      nextSampleOffset =
        ...
        (patterns[patternIndex][0].length -
          2 -
          (!notFirstBeat as unknown as number)) *
          beatLength;
      // for each beat in pattern, plus one extra if end of sequence
      isSequenceEnd = (sequenceIndex ==
        sequence.length - 1) as unknown as number;
      for (
        ...
        stop =
          ...
          ((instrument != (patternChannel[0] || 0)) as unknown as number) |
            (note as number) |
            0;
        ...
        if (note) {
          ...
          if ((note |= 0)) {
            ...
            sampleBuffer = sampleCache[
              `i${(instrument =
                patternChannel[(sampleOffset = 0)] || 0)}n${note}`
            ] =
